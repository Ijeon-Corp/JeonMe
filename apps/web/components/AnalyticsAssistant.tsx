...
export default function AnalyticsAssistant() {
  ...
  async function handleAsk(q: string) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t("dashboard.components.analyticsAssistant.errorAsk"));
    } finally {
      setAsking(false);
    }
  }
  ...
  return (
    <section className="glass mt-4 rounded-2xl p-5 shadow-card">
      ...
      <form onSubmit={handleSubmit} className="mt-3 flex gap-1.5">
        <input
          type="text"
          placeholder={t("dashboard.components.analyticsAssistant.inputPlaceholder")}
          value={question}
        ...
        >
          {asking ? "..." : t("dashboard.components.analyticsAssistant.submitButton")}
        </button>
      </form>
    </section>
  );
