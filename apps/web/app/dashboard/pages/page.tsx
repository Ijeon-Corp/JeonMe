  ...
  return (
    <div className="mx-auto max-w-3xl">
      <p className="mt-1 text-sm text-muted">
        Kelola beberapa halaman terpisah dari satu akun di jeon.id/{"{username}"}/{"{slug}"}: halaman{" "}
      ...
      <div className="glass mt-6 rounded-3xl p-5 shadow-card">
        {!canCreateAny ? (
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-ink">Slug URL (jeon.id/{username || "..."}/...)</label>
              {isFirstProdukPage ? (
                <>
                  <div className="w-full rounded-lg border border-border bg-ink/5 px-3 py-2 text-sm text-muted">
                    jeon.id/{username || "..."}/{username || "..."}
                  </div>
