...
export default function DashboardLinksPage() {
  ...
  // terkait"-nya sedang terbuka (null = tertutup semua), `nestedUploadingFor`
  // menyimpan URL foto utama yang sedang proses unggah anaknya. Dua-duanya
  // keyed by URL foto (bukan id link) karena satu blok gallery bisa punya
  // banyak foto utama, masing-masing dengan sub-galerinya sendiri.
  ...
  async function handleGalleryImageUpload(e: React.ChangeEvent<HTMLInputElement>, link: LinkItem) {
    ...
    reportMultiUpload(outcome, maxGalleryImages, "dashboard.pages.links.errors.uploadGalleryPhotoFailed");
  }
  ...
  // handleNestedImageUpload/Delete -- "foto di dalam foto" (permintaan
  // langsung pengguna, 21 September 2026), pola APA ADANYA dari
  // handleGalleryImageUpload/Delete di atas -- backend mengembalikan PETA
  // `nested_images` TERBARU (bukan cuma array satu parent) supaya cukup
  // ditimpa langsung, sama filosofinya dgn `images`.
  async function handleNestedImageUpload(e: React.ChangeEvent<HTMLInputElement>, link: LinkItem, parentUrl: string) {
    ...
    setUploadProgress(null);
    reportMultiUpload(outcome, maxNestedGalleryImages, "dashboard.pages.links.errors.uploadNestedPhotoFailed");
  }
                  ...
                  {((link.block_data?.images as string[]) ?? []).length > 0 && (
                    <div className="flex flex-col gap-1.5">
                      {((link.block_data?.images as string[]) ?? []).map((src, i) => {
                        ...
                        // "Foto di dalam foto" -- foto ANAK milik foto utama
                        // ini (keyed by URL, lihat block_data.nestedImages di
                        // links.go). Panel kelolanya baru muncul kalau
                        // nestedPanelOpenFor === src (satu panel terbuka per
                        // waktu, hindari daftar jadi terlalu panjang kalau
                        // foto banyak).
