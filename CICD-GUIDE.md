...
| 11 | `golangci-lint` gagal: "the Go language version (go1.24) used to build golangci-lint is lower than the targeted Go version (1.25.0)" | `install-mode: binary` (default) mengunduh rilis prakompilasi golangci-lint v1.64.8 yang dibangun dengan go1.24 -- `go.mod` mensyaratkan go1.25.0 (naik otomatis saat upgrade dependency di bug #8), dan golangci-lint menolak menganalisis modul yang menargetkan versi Go lebih baru dari toolchain pembangunnya sendiri | Ganti `install-mode: goinstall` — build dari source pakai toolchain Go 1.25 yang sudah disiapkan `actions/setup-go`, jadi versi build selalu cocok |
| 12 | Service baru (`minio`) yang ditambahkan ke `docker-compose.staging.yml`/`.prod.yml` tidak pernah muncul di VPS walau deploy "sukses" dan `version` di `/api/health` sudah benar | `docker-compose.*.yml` di VPS ditaruh **manual sekali** saat setup awal (SETUP-GUIDE.md) dan **tidak pernah disinkron ulang** -- `deploy-staging.yml`/`deploy-production.yml` cuma `sed` `IMAGE_TAG` di `.env`, tidak pernah menyalin ulang file compose itu sendiri. Ketahuan lewat inspeksi manual VPS (`diff` compose lokal vs remote), bukan dari Actions -- deploy tetap "berhasil" karena image API baru tidak *hard-fail* tanpa MinIO (config S3 soft-default) | Tambah `actions/checkout@v4` (dengan `ref:` persis di commit/tag yang di-deploy, penting untuk skenario rollback) + step `appleboy/scp-action` yang menyalin `docker-compose.staging.yml`/`.prod.yml` ke VPS di setiap run, sebelum `docker compose pull`/`up -d` |
| 13 | Upload produk selalu balas 503 "object storage belum dikonfigurasi" walau MinIO sudah hidup & sehat | `S3_ENDPOINT` diisi dengan skema penuh (`http://minio:9000`) -- `minio-go/v7` menolak endpoint yang mengandung skema ("Endpoint url cannot have fully qualified paths"); skema seharusnya diatur lewat opsi `Secure`/`S3_USE_SSL`, bukan di string endpoint | Ganti jadi `host:port` polos (`minio:9000`) di `.env` VPS &amp; nilai default `config.go`/`.env.example` |
| 14 | Deploy gagal "port is already allocated" saat MinIO mencoba start: `Bind for 127.0.0.1:9000 failed` | VPS ini **shared** dengan tenant lain yang kebetulan sudah pakai host port 9000/9001 untuk MinIO mereka sendiri (`ijeon-minio` milik `api.ijeon.com`, `vc-minio` milik proyek videoconf) -- compose Jeonme mem-publish port host yang sama padahal aplikasi Jeonme sendiri tidak pernah butuh akses MinIO dari host/publik (`api` menjangkaunya lewat jaringan Docker internal, host "minio") | Hapus seluruh `ports:` dari service `minio` di `docker-compose.staging.yml`/`.prod.yml` -- tidak ada port host yang dipublikasikan sama sekali, hanya dapat diakses lewat jaringan internal antar-container |
| 15 | Setelah bug #14 diperbaiki, upload/aktivasi produk berhasil tapi tautan unduhan (signed URL, REQ-F-304) tidak bisa dibuka sama sekali dari luar -- URL-nya `http://minio:9000/...` | Sesuai desain bug #14, MinIO memang tidak diekspos ke host/publik -- tapi ternyata signed URL yang dihasilkan minio-go SELALU pakai `S3_ENDPOINT` yang dikonfigurasi, jadi kalau endpoint-nya "minio:9000" (internal Docker), URL yang di-generate juga tidak bisa dijangkau siapa pun di luar jaringan Docker VPS itu sendiri | Ikuti pola yang sudah ada di VPS ini untuk proyek lain (`storage.ijeon.com`, `storage.pesentiket.com`): subdomain khusus per environment (`storage.jeonme.com`, `storage-staging.jeonme.com`) di-proxy Apache dengan TLS ke port lokal MinIO yang tidak bentrok (29000/29100, bind 127.0.0.1 saja), `S3_ENDPOINT` diisi domain publik itu (bukan "minio:9000" ataupun host:port lokal) |

| 16 | **PRODUCTION** sempat login gagal total untuk semua user ("email atau password salah" walau kredensial benar) + endpoint laporan 500 setelah rilis Sprint 6 (v0.4.0) | Deploy Production gagal (lihat bug #17 -- image belum tersedia di GHCR saat job jalan), lalu diselesaikan MANUAL dengan `docker compose up -d` langsung di VPS -- tapi langkah manual itu TIDAK menyertakan `docker compose run --rm api migrate up` yang seharusnya jalan duluan. Akibatnya image baru (query-nya sudah menyertakan kolom `suspended_at` & tabel `reports`) jalan melawan skema lama (migrasi #5 belum diterapkan) -- `AuthHandler.Login` menelan SEMUA error query sebagai "email/password salah", bukan cuma "user tidak ditemukan", jadi kegagalan skema tidak kelihatan sebagai error database, melainkan seolah-olah kredensial salah | Migrasi dijalankan manual (`docker compose run --rm api migrate up`) begitu ketahuan, login pulih dalam hitungan menit. Pelajaran: **kalau terpaksa menyelesaikan deploy yang macet secara manual, WAJIB jalankan urutan lengkap (migrate dulu, baru up -d)**, tidak cukup cuma `up -d` saja |
| 17 | `Deploy Production` gagal eksplisit: `Error failed to resolve reference "ghcr.io/.../api:<sha>": ... not found` -- padahal commit-nya baru saja di-push dan tidak ada yang salah dengan kredensial/koneksi VPS | **Race condition**: workflow ini terpicu LANGSUNG oleh push tag (`on: push: tags:`), TANPA menunggu `ci.yml` (yang build & push image ke GHCR) selesai -- beda dari `deploy-staging.yml` yang memang menunggu lewat `workflow_run`. Kalau tag rilis dibuat segera setelah push ke `main` (pola rilis yang wajar -- push lalu langsung `git tag && git push`), deploy production bisa mulai duluan sebelum image-nya selesai di-build sama sekali | Tambah step "Tunggu image tersedia di GHCR" (poll `docker manifest inspect` tiap 20 detik, maks 10 menit) sebelum checkout/deploy, di KEDUA workflow deploy (staging juga, untuk jalur `workflow_dispatch` manual yang sama-sama bisa balapan) |
| 18 | **KRITIS, PRODUCTION + STAGING**: registrasi/login/semua panggilan API dari browser 404/gagal total bagi PENGUNJUNG SUNGGUHAN sejak rilis pertama -- baru ketahuan lewat laporan user yang membuka jeonme.com asli, bukan dari testing manapun sebelumnya | `NEXT_PUBLIC_API_BASE_URL` adalah nilai **build-time** Next.js -- ter-bake permanen ke bundle JS browser saat `npm run build`, TIDAK bisa diubah lagi lewat `environment:` container saat runtime. `ci.yml` tidak pernah mengirim build-arg ini saat membangun image `web`, jadi bundle yang di-deploy SELALU jatuh ke fallback kode `"http://localhost:8080/api/v1"` -- browser pengunjung sungguhan mencoba menghubungi *localhost milik mereka sendiri*, bukan backend jeonme.com/staging.jeonme.com. Tidak pernah ketahuan sebelumnya karena SEMUA verifikasi browser sepanjang proyek ini (tanpa disadari) dijalankan lewat proses lokal (`go run .` + `npm run dev` di host yang sama, TANPA Docker), di mana "localhost:8080" kebetulan selalu benar | `lib/api-client.ts`: browser default ke path RELATIF `"/api/v1"` (Apache sudah meneruskan `/api/` ke backend di origin yang SAMA, lihat `ProxyPass /api/` di atas) -- otomatis benar di staging MAUPUN production dari SATU image yang sama, tidak perlu build-arg per environment. SSR (jalan di dalam container `web`) pakai `INTERNAL_API_BASE_URL` baru (nama service Docker `api:8080`, dibaca saat runtime karena bukan prefix `NEXT_PUBLIC_`). `next.config.js` dapat `rewrites()` sebagai jaring pengaman untuk `docker compose up` lokal tanpa Apache di depannya. **Pelajaran penting**: verifikasi "browser end-to-end" HARUS sesekali dilakukan lewat browser sungguhan yang membuka domain staging/production asli, bukan cuma localhost -- bug build-time seperti ini tidak akan pernah kelihatan dari testing lokal semulus apa pun |
| 19 | Deploy Staging gagal di step **Health check** (`curl ... /api/health` tidak pernah dapat respons sama sekali, bukan cuma versi tidak cocok) setelah rilis Modul Settings (commit `4baaad3`) -- step migrasi & step "Deploy via SSH" sendiri sama-sama SUKSES, jadi tidak kelihatan di ringkasan run mana yang salah | Commit tsb memperkenalkan `ENCRYPTION_KEY` (AES-256-GCM, lihat `internal/crypto`) sebagai config **wajib PERSIS 32 byte**, di-fail-fast lewat `mustGetEnvInProd` begitu `APP_ENV=production` -- dan `.env` VPS staging (dibuat dari `.env.example` yang `APP_ENV`-nya memang `production` walau environment-nya staging, lihat §"variabel .env" di atas) TIDAK PERNAH punya var ini karena baru diperkenalkan commit ini. `config.Load()` memanggil `log.Fatalf` begitu container start → API **crash-loop** terus-menerus → Apache reverse proxy menjawab 503 "Service Unavailable" untuk semua request `/api/*` (container tidak pernah benar-benar mendengarkan port), sementara `web` tetap sehat (200) karena tidak butuh var ini. Diagnosis dilakukan TANPA akses SSH VPS -- cukup dari API publik GitHub Actions (`/repos/.../actions/runs`, `/jobs` -- tanpa token, berhasil untuk repo ini) untuk melihat step mana yang gagal, ditambah `curl -v` langsung ke domain staging untuk konfirmasi 503 dari Apache (bukan dari aplikasi) | Tambah langkah idempotent di `deploy-staging.yml` DAN `deploy-production.yml` (sebelum `migrate up`): kalau `.env` belum punya baris `ENCRYPTION_KEY=`, generate sekali (`openssl rand -hex 16`) lalu `>> .env` permanen. **Pelajaran**: config baru yang dijaga `mustGetEnvInProd`/`mustGetEnv` HARUS ditambahkan ke `.env.example` DAN dipikirkan jalur provisioning-nya di kedua workflow deploy pada PR yang sama -- kalau tidak, deploy akan crash-loop diam-diam sampai ada yang sadar cek health check |

| 19 | Sampul produk yang baru diunggah/diaktifkan kadang "tidak tampil" di halaman publik walau API mengembalikan `cover_image_url` yang benar | `page:<username>` cache Redis (TTL 30 detik, lihat `PageHandler.GetPublicPage`) TIDAK PERNAH diinvalidasi oleh `ProductHandler` -- cuma `UpdateMyPage` & `UploadAvatar` yang melakukannya. Produk baru/sampul baru bisa nyangkut di versi cache lama sampai kedaluwarsa sendiri | Tambah `ProductHandler.invalidatePageCache()` (pola sama seperti di `PageHandler`), dipanggil di KELIMA handler yang mengubah data produk: `Create`, `Update`, `Delete`, `UploadFile`, `UploadCover`. Ditemukan sekaligus dengan menambahkan fitur sampul produk (`UploadCover`) -- `cover_image_url` sebelumnya memang tidak pernah bisa diisi sama sekali (tidak ada endpoint upload), jadi bug cache ini baru kelihatan begitu jalur upload-nya ada |

Bug #1–6 diperbaiki di commit `edae5a8` s.d. `f6c6010`; #7–10 di commit `47c51f4` s.d. `c421ab0`; #11 di commit `d6ff131`; #12–15 ditemukan &amp; diperbaiki saat rollout Sprint 2 (integrasi MinIO); #16–17 ditemukan &amp; diperbaiki saat rilis Sprint 6 (v0.4.0) ke production -- #17 baru ketahuan akar masalahnya belakangan lewat log Actions asli yang dibagikan user (sebelumnya dikira "macet", ternyata gagal eksplisit karena race condition); #18 ditemukan 16 Juli 2026 lewat laporan user yang membuka jeonme.com asli dan gagal register -- bug paling parah dalam katalog ini (menonaktifkan fungsi inti untuk SEMUA pengunjung sejak awal), diperbaiki & di-deploy ke staging+production (v0.7.0) dalam hitungan menit setelah ditemukan; #19 ditemukan sesaat setelah itu lewat laporan user "gambar produk tidak tampil" -- ternyata dua masalah sekaligus (fitur sampul yang belum pernah ada, DAN bug cache independen yang baru kelihatan begitu fitur sampulnya dibangun). Semuanya lewat verifikasi langsung di staging/production sungguhan, bukan asumsi dari log CI atau testing lokal saja.
