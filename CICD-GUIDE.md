...
| 16 | **PRODUCTION** sempat login gagal total untuk semua user ("email atau password salah" walau kredensial benar) + endpoint laporan 500 setelah rilis Sprint 6 (v0.4.0) | `Deploy Production` (workflow_run) tersendat di step "Deploy via SSH" (image sudah ke-pull, tapi `up -d` tidak sempat jalan). Diperbaiki manual dengan `docker compose up -d` langsung -- TAPI langkah manual itu TIDAK menyertakan `docker compose run --rm api migrate up` yang seharusnya jalan duluan. Akibatnya image baru (yang query-nya sudah menyertakan kolom `suspended_at` & tabel `reports`) jalan melawan skema lama (migrasi #5 belum diterapkan) -- `AuthHandler.Login` menelan SEMUA error query sebagai "email/password salah", bukan cuma "user tidak ditemukan", jadi kegagalan skema tidak kelihatan sebagai error database, melainkan seolah-olah kredensial salah | Migrasi dijalankan manual (`docker compose run --rm api migrate up`) begitu ketahuan, login pulih dalam hitungan menit. Pelajaran: **kalau terpaksa menyelesaikan deploy yang macet secara manual, WAJIB jalankan urutan lengkap (migrate dulu, baru up -d)**, tidak cukup cuma `up -d` saja -- staging tidak kena masalah yang sama karena deploy otomatisnya kebetulan selalu selesai penuh di sana |

Bug #1–6 diperbaiki di commit `edae5a8` s.d. `f6c6010`; #7–10 di commit `47c51f4` s.d. `c421ab0`; #11 di commit `d6ff131`; #12–15 ditemukan &amp; diperbaiki saat rollout Sprint 2 (integrasi MinIO); #16 ditemukan &amp; diperbaiki saat rilis Sprint 6 (v0.4.0) ke production, semuanya lewat verifikasi langsung di staging/production sungguhan, bukan asumsi dari log CI saja.
